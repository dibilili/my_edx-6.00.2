...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        if all([self.isResistantTo(drug) for drug in activeDrugs]) is True:
            if random.random() <= (self.maxBirthProb * (1 - popDensity)):
                new_resistance = self.resistances.copy()
                for i in self.resistances.keys():
                    if random.random() <= self.mutProb:
                        new_resistance[i] = not self.resistances[i]
...
class TreatedPatient(Patient):
    ...
    def getResistPop(self, drugResist):
        ...
        for i in self.viruses:
            if all([i.isResistantTo(j) for j in self.drugResist]) is True:
                pop += 1
        return pop
    ...
    def update(self):
        ...
        for j in viruses_copy_2:
            try:
                j.reproduce(popDensity, self.drugs)
